...
class Inicio
{
    const BUSCAR_TODOS = 'SELECT * FROM 1pesquisa ORDER BY ocorrencia_contato_nome';
    const BUSCAR_POR_ID = 'SELECT * FROM 1pesquisa WHERE id = ?';
    const INSERIR = 'INSERT INTO 1pesquisa(ocorrencia_contato_nome,ocorrencia_contato_fone) VALUES (?, ?)';
    const INSERIR_ELEITOR = 'INSERT INTO votos(id_candidato,eleitor) VALUES (?, ?)';
    const DELETAR_POR_ID = 'DELETE FROM 1pesquisa WHERE id = ?';
    const ATUALIZAR_POR_ID = 'UPDATE 1pesquisa SET nome = ?, descricao = ?, votos = ? WHERE id = ?';
    private $id;
    private $nome;
    private $descricao;
    private $votos;
